      ...
      describe('lists.public', () => {
        it('sends all public lists', (done) => {
          ...
          collector.collect('lists.public', (collections) => {
            chai.assert.equal(collections.Sessions.length, 3);
            done();
          });
        });
      });
      ...
      describe('lists.private', () => {
        it('sends all owned lists', (done) => {
          ...
          collector.collect('lists.private', (collections) => {
            chai.assert.equal(collections.Sessions.length, 2);
            done();
          });
        });
      });
    ...
    describe('methods', () => {
      ...
      describe('makePrivate / makePublic', () => {
        function assertListAndTodoArePrivate() {
          assert.equal(Sessions.findOne(listId).userId, userId);
          assert.isTrue(Sessions.findOne(listId).isPrivate());
        ...
        it('makes a list private and updates the todos', () => {
          ...
          assert.isFalse(Sessions.findOne(listId).isPrivate());
      ...
      describe('updateName', () => {
        it('changes the name, but not if you don\'t have permission', () => {
          ...
          assert.equal(Sessions.findOne(listId).name, 'new name');
          ...
          assert.equal(Sessions.findOne(listId).name, 'new name 2');
