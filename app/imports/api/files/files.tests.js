      ...
      describe('lists.public', () => {
        it('sends all public lists', (done) => {
          ...
          collector.collect('lists.public', (collections) => {
            chai.assert.equal(collections.Files.length, 3);
            done();
          });
        });
      });
      ...
      describe('lists.private', () => {
        it('sends all owned lists', (done) => {
          ...
          collector.collect('lists.private', (collections) => {
            chai.assert.equal(collections.Files.length, 2);
            done();
          });
        });
      });
    ...
    describe('methods', () => {
      ...
      describe('makePrivate / makePublic', () => {
        function assertListAndTodoArePrivate() {
          assert.equal(Files.findOne(listId).userId, userId);
          assert.isTrue(Files.findOne(listId).isPrivate());
        ...
        it('makes a list private and updates the todos', () => {
          ...
          assert.isFalse(Files.findOne(listId).isPrivate());
      ...
      describe('updateName', () => {
        it('changes the name, but not if you don\'t have permission', () => {
          ...
          assert.equal(Files.findOne(listId).name, 'new name');
          ...
          assert.equal(Files.findOne(listId).name, 'new name 2');
