...
class ReviewPage extends BaseComponent {
  ...
  handleGenerate = () => {
    ...
    toast(() => <AppNotification msg={"transcript started"} desc={"processing discussion audio..."} icon={"spinner"} />, {
      autoClose: 8000
    });
  ...
  renderSlideTags = (selected, done: false) => {
    ...
    if (selected.length === 0) {
      ...
      const slideKeys = slideNos.map(s => (
        <kbd iter={s.slideNo} className={active(s.slideNo)} key={`key-${s.slideNo}`} data-file-id={s.slideId} onClick={this.setBySlide}>
          {s.slideNo}
        </kbd>
      ));
      return (
  ...
  renderCommentFilter = () => {
    ...
    const slideSort = setSort(x => (x.slides[0] ? Number(x.slides[0].slideNo) : Infinity), "slide", false);

    return (
      <div>
        <div className="btn-m-straight btn-m-group btns-group">
          <button onClick={timeSort} className={"btn btn-menu" + (filter === "time" ? " active" : "")}>
            time
          </button>
          <button className={"btn btn-menu" + (filter === "slide" ? " active" : "")} onClick={slideSort}>
            slide
          </button>
          <button className={"btn btn-menu" + (filter === "auth" ? " active" : "")} onClick={authSort}>
            auth
          </button>
          <button className={"btn btn-menu" + (filter === "agree" ? " active" : "")} onClick={agreeSort}>
            agree
          </button>
          <button className={"btn btn-menu" + (filter === "flag" ? " active" : "")} onClick={flagSort}>
            discuss
          </button>
  ...
  renderGenerate = () => {
    ...
    return (
      <div>
        <AlertLink handleClick={this.handleGenerate} text={"generate transcript"} center={true} />
        {soundDownload}
      </div>
    );
  };
    ...
    if (!comments || !comments.length) {
      ...
      return items.length > 0 ? (
        <div>
          ...
          <CommentList title={"addressed"} items={completed} defaultOpen={false} />
        </div>
      ) : (
    ...
    return (
      <div className="context-filter">
        <span className="list-title list-title-basic">
          ...
          {sound && WaveSurfer && (
            <button className="btn btn-menu pull-right btn-empty" onClick={this.handleExtra}>
              more
            </button>
          )}
        </span>
        ...
        <div id="grid-holder">
          <div className="alert centered no-margin">
            use <i className="fa fa-arrow-left" /> and <i className="fa fa-arrow-right" /> to filter by slide
          </div>
  ...
  renderSounds = () => {
    ...
    return (
      <div className="float-at-top">
        <Waveform src={src} ref={this.waveRef} region={activeRegion} handleAudioSet={this.setDuration} />
      </div>
    );
  };
  ...
  renderTranscript = () => {
    ...
    return <CommentList title={"transcript"} content={content} defaultOpen={true} />;
  };
  ...
  generateBaseComments = () => {
    ...
    let csort = _.orderBy(comments, [sorter, "created"], [invert ? "desc" : "asc", "asc"]);
  ...
  generateWordList = (region, pad = 200) => {
    ...
    return results.filter(word => {
      return word.startTime > (startTime - pad) / 1000 && word.endTime < (stopTime + pad) / 1000;
    });
  };
  ...
  renderRegions = () => {
    ...
    let rsort = _.orderBy(filtered, [sorter, "startTime"], [invert ? "desc" : "asc", "asc"]);
